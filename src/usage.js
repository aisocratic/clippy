...
async function lastAssistantText(transcriptPath, { maxChars = 600 } = {}) {
  ...
  // agent_message event. Prefer the latter and let the newest one win.
  for (const line of text.split('\n')) {
    if (!line || line.indexOf('"agent_message"') === -1) continue;
    let entry;
    try {
      entry = JSON.parse(line);
    } catch {
    ...
    const message = entry.type === 'event_msg' && entry.payload?.type === 'agent_message'
      ? entry.payload.message
      : '';
    if (typeof message === 'string' && message.trim()) recap = message.trim();
  }
